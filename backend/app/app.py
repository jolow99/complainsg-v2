...
async def chat_endpoint(
    ...
    """
    Chat endpoint that processes user messages and saves to database.
    """
    ...
    if request.conversation_id:
        ...
        # Use provided conversation history or empty list
        conversation_history = [
    ...
    if request.conversation_id:
        ...
        await add_message_to_conversation(
            db, request.conversation_id,
            MessageCreate(role="assistant", content=result["response"]),
            user.id
        )
        ...
        conversation = await create_conversation_with_messages(
            db, user.id, request.message, result["response"]
        )
...
async def public_chat_endpoint(request: ChatRequest):
    ...
    Note: This doesn't save conversations to database.
    """
    ...
    # Call LLM directly for public endpoint too
    from agent.utils import stream_llm
    import os
    ...
    updated_history = conversation_history + [{"user": request.message, "assistant": full_response}]
